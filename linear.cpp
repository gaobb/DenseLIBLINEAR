...
*/
static void solve_l2r_l1l2_svc(
	const int num_examples,const int num_dim,float** data, int* labels, double *w, double eps,
	double Cp, double Cn, int solver_type)
{
	int l = num_examples;
	int w_size = num_dim;
	...
	for(i=0; i<l; i++)
	{
		...
		{
			QD[i] += data[i][j]*data[i][j];
			w[j] += y[i]*alpha[i]*data[i][j];
		}
		index[i] = i;
	}
	...
	while(newton_iter < max_newton_iter)
	{
		...
		while(iter < max_iter)
		{
			...
			for(s=0; s<QP_active_size; s++)
			{
				...
				QP_Gnorm1_new += violation;:8: note: decla
...
static void group_classes(const int num_examples,int* labels, int *nr_class_ret, int **label_ret, int **start_ret, int **count_ret, int *perm)
{
	int l = num_examples;
	int max_nr_class = 16;
	int nr_class = 0;
...
static void train_one(const int num_examples,const int num_dim,float** data, int* labels,const parameter *param, double *w, double Cp, double Cn)
{
	...
	//double primal_solver_tol = eps*max(min(pos,neg), 1)/num_examples;
	...
	{       /*
		...
		case L2R_L2LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(num_examples,num_dim,data,labels, w, eps, Cp, Cn, L2R_L2LOSS_SVC_DUAL);
			break;
		case L2R_L1LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(num_examples,num_dim,data,labels, w, eps, Cp, Cn, L2R_L1LOSS_SVC_DUAL);
			break;
                /*
		case L1R_L2LOSS_SVC:
		{
		...
		case L1R_LR:
		{
			...
			break;
                ...
                */
		case L2R_LR_DUAL:
			solve_l2r_lr_dual(num_examples,num_dim,data,labels, w, eps, Cp, Cn);
			break;
                /*
		case L2R_L2LOSS_SVR:
		{
...
model* train(const int num_examples,const int num_dim,float** data, int* labels,const parameter *param)
{
	int i,j;
	int l = num_examples;
	int n = num_dim;
	...
	if(check_regression_model(model_))
	{
		...
		//train_one(prob, param, model_->w, 0, 0);
                train_one(num_examples,num_dim,data,labels, param, model_->w, 0, 0);
	}
	else
	{
		int nr_class;
		...
		//group_classes(prob,&nr_class,&label,&start,&count,perm);
                group_classes(num_examples,labels,&nr_class,&label,&start,&count,perm);
		...
		{
			if(nr_class == 2)
			{       float** subdata = Malloc(float*, l);
	                        int* sublabels = Malloc(int, l);
				...
				train_one(num_examples,num_dim,subdata,sublabels, param, model_->w, weighted_C[0], weighted_C[1]);
			}
			else
			{
				...
				for(i=0;i<nr_class;i++)
				{
					...
					train_one(num_examples,num_dim,subdata,sublabels, param, w, weighted_C[i], param->C);

					for(int j=0;j<w_size;j++)
						model_->w[j*nr_class+i] = w[j];
                                        int svm_time = EndOfDuration();
                                        info("\ntraining the %dth linear svm model in %d MilliSeconds\n",i+1,svm_time);
                                        free(subdata);
                                        free(sublabels);
                                        free(w);
				}
...
void mysort(node s[], int l, int r)
{
    ...
    if (l<r)
    {
        ...
        while (i<j)
        {
            while(i<j && s[j].value<x.value)
		j--; /* ŽÓÓÒÏò×óÕÒµÚÒ»žöÐ¡ÓÚxµÄÊý */
            if(i<j)
		s[i++] = s[j];
            while(i<j && s[i].value>x.value)
		i++; /* ŽÓ×óÏòÓÒÕÒµÚÒ»žöŽóÓÚxµÄÊý */
            if(i<j)
		s[j--] = s[i];
        }
        s[i] = x;
        mysort(s,l,i-1); /* µÝ¹éµ÷ÓÃ */
        mysort(s,i+1,r);
    }
}

double predict_values(const struct model *model_, const int num_dim, const float *fv, double *dec_values,int top5[])
{
	//int idx;
	int n;
	if(model_->bias>=0)
	...
	else
	{
		int dec_max_idx = 0;
                ...
                for (i=0;i<nr_class;i++)
                {
                    score[i].value = dec_values[i];
                    score[i].index = i;
                }
               
                mysort(score,0,nr_class-1);
                for (i=0;i<5;i++)
                {
                  //info("%4d  ",model_->label[score[i].index]);
                  top5[i] = model_->label[score[i].index];
                  //info("%4d  ",top5[i]);
                }
                /*
		for(i=1;i<nr_class;i++)
		{
			if(dec_values[i] > dec_values[dec_max_idx])
                ...
                */
                dec_max_idx = score[0].index;
		return model_->label[dec_max_idx];
	}
}

double predict(const model *model_, const int num_dim, const float *fv,int top5[])
{
	double *dec_values = Malloc(double, model_->nr_class);
	double label=predict_values(model_, num_dim, fv, dec_values,top5);
	free(dec_values);
	return label;
}

double predict_probability(const struct model *model_, const int num_dim, const float *fv, double* prob_estimates,int top5[])
{
	if(check_probability_model(model_))
	{
		...
		double label=predict_values(model_, num_dim, fv, prob_estimates,top5);
